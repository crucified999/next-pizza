...
export const SearchInputMenuUI: React.FC<SearchInputMenuUIProps> = ({ items, isFocused }) => {
  return (
    <div className={cn(
      ...
      <ul className="flex flex-col flex-1 gap-5 px-5">
        {items.map((item) => (
          <SearchInputMenuItemUI key={item.title} title={item.title} image={item.image} />
        ))}
      </ul>
    </div>
  );
