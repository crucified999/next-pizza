    ...
    <div className="grid grid-cols-[1.3fr_1fr]">
      ...
      <div className="p-5 bg-[#fafafa] rounded-tr-xl rounded-br-xl">
        ...
        <div className="flex flex-col gap2">
          <ChooseForm onClick={(value) => setSize(Number(value) as PizzaSize)} formType="Размер" items={product.items} ingredients={product.ingredients} />
          <ChooseForm onClick={(value) => setType(Number(value) as PizzaType)} formType="Тесто" items={product.items} ingredients={product.ingredients} />
        </div>
        <div className="p-3">
        ...
        <div className="flex place-items-center py-2">
          <button className="flex-1 bg-orange-500 transition-colors duration-150 hover:bg-orange-600 rounded-2xl p-3 cursor-pointer">
            <span className="text-white">В корзину за {product.items[0].price} ₽</span>
          </button>
        </div>
      </div>
    </div>
